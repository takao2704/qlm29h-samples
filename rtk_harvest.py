#!/usr/bin/env python3
"""
rtk_harvest.py  –  NTRIP client + SORACOM Harvest Data sender for Quectel QLM29HBAA-GM

Receives RTCM corrections from Quectel RTK service, injects them into the
GNSS module via USB serial, and periodically sends parsed GNSS data to
SORACOM Harvest Data via Unified Endpoint.
...
Usage:
    1. Set NTRIP_USER and NTRIP_PASS as environment variables with the
       credentials provided by SORACOM:
           export NTRIP_USER="..."
           export NTRIP_PASS="..."
    2. Run: python3 rtk_harvest.py

Notes:
    - Harvest Data is sent via SORACOM SIM (USB dongle etc.)
    - No authentication required when posting via SORACOM network
    - Unified Endpoint: http://unified.soracom.io
"""
...
HARVEST_ENDPOINT = "http://unified.soracom.io"  # SORACOM Unified Endpoint
HARVEST_INTERVAL = 5                             # Harvest Dataへの送信間隔（秒）
# ==========================================
...
def ntrip_client(ser: serial.Serial):
    ...
    global running, ntrip_connected
...
def parse_nmea_coord(value: str, direction: str) -> float:
    """
    NMEA度分形式 → 十進数度に変換
    例: "3556.391954", "N" → 35.93986590
        "13926.061108", "E" → 139.43435180
    """
    if not value:
        return None
    dot = value.index(".")
...
def parse_gga(sentence: str) -> dict:
    """
    GGAセンテンスをパースしてHarvest Data用のdictを返す。
    位置が取得できない（quality=0）場合はNoneを返す。
    """
    parts = sentence.strip().split(",")
    # $GNGGA,hhmmss.ss,lat,N,lon,E,quality,sats,hdop,alt,M,sep,M,age,ref*cs
    if len(parts) < 10:
        return None

    quality_code = parts[6]
    if quality_code == "0":
        return None  # No Fix は送らない
    ...
    try:
        ...
        utc_time = parts[1][:6]  # HHMMSS
        utc_formatted = f"{utc_time[:2]}:{utc_time[2:4]}:{utc_time[4:6]}Z" if len(utc_time) == 6 else utc_time
    ...
    payload = {
        "lat":           lat,
        "lon":           lon,
        "quality":       int(quality_code),
        "quality_label": GGA_QUALITY.get(quality_code, f"Unknown({quality_code})"),
...
def harvest_sender():
    """HARVEST_INTERVAL 秒ごとにGGAをパースしてHarvest Dataへ送信"""
    global running
    ...
    while running:
        ...
        try:
            ...
            if resp.status_code in (200, 201):
                print(f"[HARVEST] Sent: {payload}  →  HTTP {resp.status_code}")
            else:
                print(f"[HARVEST] Error: HTTP {resp.status_code}  payload={payload}")
        except Exception as e:
...
def main():
    ...
    t_harvest = threading.Thread(target=harvest_sender,             daemon=True)
    t_serial.start()
    t_ntrip.start()
    t_harvest.start()
    ...
    print(f"[MAIN] Harvest Data will be sent every {HARVEST_INTERVAL}s to {HARVEST_ENDPOINT}")
    try:
        while True:
            time.sleep(1)
