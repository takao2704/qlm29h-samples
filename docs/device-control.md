...
| `payload_config_update` | プリセット、送信間隔、NMEA文種などの送信設定を更新する |
...
{
  "version": 1,
  "request_id": "payload-config-20260715-001",
  ...
  "parameters": {
    "configuration": {
      ...
      "preset": "custom",
      "interval_sec": 10,
      "include_sentences": ["GGA", "RMC", "GSA", "GSV"]
    }
  }
}
```

`enabled`を省略すると現在の送信開始・停止状態を維持します。

## SORACOM Remote Command HTTP

`remote_command_http.py` を利用すると、SORACOM Remote CommandからJSONコマンドを配置し、現在のステータスを取得できます。

| メソッドとパス | 動作 |
|---|---|
| `POST /v1/commands` | コマンドを検証し、`device-command.json`へ配置して `202` を返す |
| `GET /v1/status` | 送信設定と `device-status.json` の現在値を返す |

受付はSORACOM AirのRemote Command送信元IPとBearerトークンを検証します。Lambda UIを含む構成とデプロイ方法は `remote_control_ui/README.md` を参照してください。
