...
対応アクションは `transmission_start`、`transmission_stop`、`dr_alignment_start`、`dr_alignment_cancel` です。処理状態と結果は `/home/pi/.config/qlm29h/device-status.json` に出力されます。DR走行校正中はシリアルポート競合を避けるため送信デーモンを一時停止し、完了・失敗・中止後に自動再開します。
